  ...
  return (
    <div className="flex flex-wrap items-center justify-between">
      ...
      <nav className={cn('rounded-t max-lg:mt-2', navClass)}>
        <ul className="flex gap-x-6 gap-y-1 text-lg font-medium max-lg:flex-col [&_a:hover]:opacity-100 [&_a]:opacity-70 max-lg:[&_a]:inline-block max-lg:[&_a]:w-full">
          {props.children}
        </ul>
      </nav>
      ...
      >
        <ul className="flex flex-row items-center gap-x-1.5 text-lg font-medium [&_li[data-fade]:hover]:opacity-100 [&_li[data-fade]]:opacity-70">
          {props.rightMenu}
        </ul>
      </div>
    </div>
